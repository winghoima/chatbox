...
ipcMain.handle('exportSession', async (event, sessionData: { name: string; data: string }) => {
    const sanitizedName = sessionData.name.replace(/[<>:"/\\|?*]/g, '-');
    ...
    try {
        fs.writeFileSync(result.filePath, sessionData.data, 'utf-8');
        return { success: true };
    } catch (error) {
